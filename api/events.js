...
function getOrCreateParticipantId(req, res) {
    ...
    res.setHeader(
        'Set-Cookie',
        `${PARTICIPANT_COOKIE}=${participantId}; Max-Age=${PARTICIPANT_COOKIE_MAX_AGE}; Path=/; HttpOnly; Secure; SameSite=Lax`
    );
...
async function databaseError(response, operation) {
    ...
    try {
        ...
        // Keep raw response text when Supabase did not return JSON.
    }

    return {
        error: `Database ${operation.toLowerCase()} failed`,
        details: parsed?.message || parsed?.hint || parsed?.details || details.slice(0, 500)
    };
}
...
function validateEvent(body) {
    ...
    const startsAt = clean(body.starts_at, 40);
    const endsAt = clean(body.ends_at, 40);
    const category = clean(body.category, 30);
    const description = clean(body.description, 500);
    const isFeatured = body.is_featured === true;
    const guestName = clean(body.featured_person_name, 100);
    const guestRole = clean(body.featured_person_role, 100);

    if (title.length < 2 || venue.length < 2 || description.length < 5) {
        return { error: 'Title, venue and description are invalid' };
    }

    if (!validIsoDateTime(startsAt) || !validIsoDateTime(endsAt)) {
    ...
    if (isFeatured && (!guestName || !guestRole)) {
    ...
    try {
        if (req.method === 'GET') {
            const response = await supabaseRequest(
                'events?select=id,title,venue,starts_at,ends_at,category,description,is_featured,featured_person_name,featured_person_role,participants,created_at&order=starts_at.asc',
                { method: 'GET' }
            );
        ...
        if (req.method === 'POST') {
            ...
            if (body.action === 'participate') {
                ...
                const eventResponse = await supabaseRequest(
                    `events?id=eq.${encodeURIComponent(eventId)}&select=id,participants,ends_at`,
                    { method: 'GET' }
                );
                ...
                if (new Date(rows[0].ends_at) <= new Date()) {
                    return json(res, 400, { error: 'This event has already ended.' });
                }

                const participationResponse = await supabaseRequest('event_participations', {
                    method: 'POST',
                ...
                if (participationResponse.status === 409) {
                    return json(res, 409, {
                        error: 'Your participation has already been recorded.',
                ...
                if (!participationResponse.ok) {
                    return json(res, 502, await databaseError(participationResponse, 'PARTICIPATION INSERT'));
                }

                const updateResponse = await supabaseRequest(`events?id=eq.${encodeURIComponent(eventId)}`, {
                    method: 'PATCH',
                    headers: { Prefer: 'return=representation' },
                    body: JSON.stringify({ participants: Number(rows[0].participants) + 1 })
                });

                if (!updateResponse.ok) {
                    return json(res, 502, await databaseError(updateResponse, 'PARTICIPANT COUNT UPDATE'));
                }
        ...
        if (req.method === 'PATCH') {
            ...
            if (body.starts_at !== undefined || body.ends_at !== undefined) {
                const current = await supabaseRequest(
                    `events?id=eq.${encodeURIComponent(id)}&select=starts_at,ends_at`,
                    { method: 'GET' }
                );
            ...
            if (body.participants !== undefined) {
                updates.participants = Math.max(0, Number(body.participants) || 0);
            }

            if (!Object.keys(updates).length) return json(res, 400, { error: 'No valid fields to update' });

            const response = await supabaseRequest(`events?id=eq.${encodeURIComponent(id)}`, {
                method: 'PATCH',
                headers: { Prefer: 'return=representation' },
