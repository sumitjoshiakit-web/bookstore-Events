...
async function supabaseRequest(path, options = {}) {
    let url = (process.env.SUPABASE_URL || '').trim().replace(/\/+$/, '').replace(/\/rest\/v1$/i, '');
...
export default async function handler(req, res) {
    try {
        if (req.method === 'GET') {
            const response = await supabaseRequest('events?select=id,title,venue,date,time,category,description,participants&order=date.asc,time.asc', { method: 'GET' });
        ...
        if (req.method === 'POST') {
            ...
            if (body.action === 'participate') {
                ...
                const eventResponse = await supabaseRequest(`events?id=eq.${encodeURIComponent(eventId)}&select=id,participants`, { method: 'GET' });
                if (!eventResponse.ok) return json(res, 502, await databaseError(eventResponse, 'READ'));
                const eventRows = await eventResponse.json();
                if (!eventRows.length) return json(res, 404, { error: 'Event not found' });
                ...
                const updateResponse = await supabaseRequest(`events?id=eq.${encodeURIComponent(eventId)}`, {
                    method: 'PATCH',
                    headers: { Prefer: 'return=representation' },
                    body: JSON.stringify({ participants: Number(eventRows[0].participants) + 1 })
                });
                ...
                return json(res, 201, { alreadyParticipated: false, event: mapEvent(updatedRows[0]) });
            }
            ...
            if (title.length < 2 || venue.length < 2 || description.length < 5) return json(res, 400, { error: 'Invalid event data' });
            if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || !/^\d{2}:\d{2}$/.test(time)) return json(res, 400, { error: 'Invalid date or time' });
            if (!ALLOWED_CATEGORIES.has(category)) return json(res, 400, { error: 'Invalid category' });

            const row = { id: crypto.randomUUID(), title, venue, date, time, category, description, participants: 0 };
